...
| D21 | Composite failure semantics: `map`/`gather`/`start_child` are fail-fast (a failed part raises through the composite, like native `await`); collect-style deferred post-MVP | Superseded by D27 | [ADR-0020](adr/0020-composite-failure-semantics.md) |
...
| D27 | **Collect-mode fan-out** (supersedes D21): `map`/`gather` take `return_exceptions=True`, fail-fast stays the default; a task failure the run *survives* is recorded as a terminal `TaskFailed` event and replays as a hit; a collected slot always holds a Satay error type (`TaskFailedError` / `WorkflowFailedError`), never the task's own class, since the journal stores a class *name* and not an import path (ADR-0005) | Accepted | [ADR-0027](adr/0027-collect-mode-fan-out.md) |
| D-products | **Products on top of Satay live in their own repositories.** The graph-DSL non-goal in the core is reaffirmed (R0.2, D25.4): no graph DSL, no node registry, no graph interpreter in `satay`. A **pipeline-graph builder is sanctioned as a separate product** that **compiles down to readable `@workflow`/`@task` Python** — one-way coupling, Satay never learns what a graph is. It edits a control-flow graph (loops included), not a DAG; it launches on the agentic workload class, with ML as submit-and-poll and ETL as push-down only; and it starts in earnest after the D25 launch cards, the D26 tier-1 plane, and ARCHITECTURE §9 phases 2–3. Hosted execution stays tier 3 and undecided | Accepted | [ADR-0032](adr/0032-products-on-top-pipeline-graph-builder.md) |
